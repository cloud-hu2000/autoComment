(function () {
  ...
  let _batchCtx = null; // { batchId, urlId, url }

  function setBatchContext(batchId, urlId, url) {
    _batchCtx = { batchId, urlId, url };
    // 同步写入 storage，刷新后 initOnPageReady 能读到
    if (typeof chrome !== 'undefined' && chrome.storage) {
      chrome.storage.local.set({
        batchCtx: { batchId, urlId, url, timestamp: Date.now() }
      }, () => {});
    }
  }
  ...
  async function restoreBatchContext() {
    ...
    if (ctx && ctx.batchId && ctx.urlId) {
      _batchCtx = { batchId: ctx.batchId, urlId: ctx.urlId, url: ctx.url };
      // 清理已超过10分钟的过期上下文
  ...
  async function reportSuccessToBatch(aiContent) {
    if (!_batchCtx) return;
    const { batchId, urlId, url } = _batchCtx;
    try {
  ...
  if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.onMessage) {
    chrome.runtime.onMessage.addListener((message, _sender, _sendResponse) => {
      ...
      if (message && message.type === 'BATCH_HANDLE') {
        setBatchContext(message.batchId, message.urlId, message.url);
