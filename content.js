(function () {
  ...
  async function restoreBatchContext() {
    ...
    if (ctx && ctx.batchId && ctx.urlIndex !== undefined) {
      ...
      // 重置状态标志，确保在新 URL 上正确检测评论框
      autoGeneratedOnce = false;
      hasNotifiedCommentBox = false;
      hasCheckedInitialCommentBox = false;
      hasTriggeredCommentFlow = false;
      console.log('[AutoComment] restoreBatchContext 已重置状态标志');
      // 清理已超过10分钟的过期上下文
